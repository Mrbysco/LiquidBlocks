...
public class LiquidDatagen {
	...
	public static class Recipes extends RecipeProvider {
		...
		protected void buildCraftingRecipes(Consumer<FinishedRecipe> recipeConsumer) {
			...
			ShapelessRecipeNoRemainderBuilder.shapeless((Item) LiquidRegistry.LIQUID_ORE.getBucketRegistry().get())
		...
		private void buildWaterRecipes(LiquidBlockReg reg, Block block, Consumer<FinishedRecipe> recipeConsumer) {
			...
			new ConditionalRecipe.Builder()
					...
					.addRecipe(
							ShapelessRecipeNoRemainderBuilder.shapeless((Item) reg.getBucketRegistry().get())
									.requires(block).requires(Items.BUCKET).requires(NBTIngredient.of(waterBottle))
